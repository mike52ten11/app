...
def call_resource_database_api_for_starttime_and_endtime(username, qrid, startdate, enddate):
    url = f"{server_IP}api/energy/"
    # url = 'http://10.52.15.201:800/api/energy/'
    params = {
        'username': username,
        'qrid':  qrid,
...
class EnergyListCreate(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **krgs):
        ...
        if len(request.GET)>0:
            ...
            request_start_date = request.GET.get('startdate','None')
            request_end_date = request.GET.get('enddate','None')
            ...
            return JsonResponse({'message':'錯誤的參數, 網址是http://API伺服器IP/api/energy/?username=&date=&qrid=, 或http://API伺服器IP/api/energy/?username=&qrid=&startdate=&enddate='},
                                json_dumps_params={'ensure_ascii': False},
                                safe=False)
        ...
        if  request_username=='None' or request_qrid=='None':
            ...
            return JsonResponse({'Errormessage':f"參數錯誤{request_username_of_Errormessage},{request_qrid_of_Errormessage}"},
                                    json_dumps_params={'ensure_ascii': False},
                                    safe=False)

        if request_start_date=='None' and request_end_date=='None':

            if request_date=='None':

                return JsonResponse({'Errormessage':"參數裡沒有 date"},
                                    json_dumps_params={'ensure_ascii': False},
                                    safe=False)
        ...
        elif request_start_date!='None' and request_end_date!='None':

            request_start_date = parse_date_of_userParasmeter_and_transfor_to_dateParasmeter_of_server(request_start_date)
            request_end_date = parse_date_of_userParasmeter_and_transfor_to_dateParasmeter_of_server(request_end_date)
            print('request_start_date with , = ',request_start_date)
            print('request_end_date with , = ',request_end_date)
            responseData = call_resource_database_api_for_starttime_and_endtime(request_username,request_qrid,request_start_date,request_end_date)
            return JsonResponse({'message':responseData}, json_dumps_params={'ensure_ascii': False},safe=False)
        else:


            return JsonResponse({'Errormessage':"參數錯誤，有date就不要給startdate和enddate;或有startdate和enddate就不要給date;startdate和enddate必須同時給"},
                    json_dumps_params={'ensure_ascii': False},
                    safe=False)
